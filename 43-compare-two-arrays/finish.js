...
// потому что это сравнение не массивов, а переменных. как мы знаем переменные хранят только ссылки на объекты, а не сами объекты
// в связи с чем, хоть массив А и схож на массив B, но они ссылаются на разные ячейки памяти, на разные объекты

const c = [2, 1, 3]
const d = [1, 2, 3, 4]

const areArraysEqual = (firstArray, secondArray) => {
    if (firstArray.length === secondArray.length && firstArray.every((el, ind) => el === secondArray[ind])) {
        return true;
    }
    return false;
}
